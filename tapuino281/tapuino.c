...
uint8_t g_motor_mask = MOTOR_MASK;	//**
...
ISR(TIMER1_COMPA_vect) {
  ...
  if ((MOTOR_IS_OFF() && g_motor_mask) || g_is_paused) { //**
    return;
  }
...
void record_file(char* pfile_name) {
  ...
  SENSE_ON();
  //while (MOTOR_IS_OFF()) {
  while (MOTOR_IS_OFF() && g_motor_mask) { //**
    if (g_cur_command == COMMAND_ABORT) {
      break;
    }
    // feedback to the user
  ...
  while (!g_tap_file_complete) {
    while ((g_read_index & 0x80) == (g_write_index & 0x80)) {
      ...
      if (MOTOR_IS_OFF() && g_motor_mask && g_rec_auto_finalize) {	//**
...
void load_eeprom_data() {
  if (eeprom_read_byte((uint8_t *) 0) == 0xB6) {
    ...
    g_motor_mask = eeprom_read_byte((uint8_t *) 9);	//**
  }
}
...
void save_eeprom_data() {
  ...
  eeprom_update_byte((uint8_t *) 9, g_motor_mask);  //**
